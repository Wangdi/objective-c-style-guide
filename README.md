# BreadTrip Objective-C Style Guide

面包旅行iOS团队所遵循的代码规范，从现有资料中整理和过往经验中总结而成。

## 概要

这篇规范是为面包旅行进行iOS开发所写，以便多人共同开发时能遵守相同的代码风格和命名规范。此规范注重于代码的清晰，可读性，命名的逻辑性。

这篇规范并没有包含所有方面和细节，大多数的行为都遵循[主要规范](#主要规范)中的说明，以下所写的是与其他规范中不同或需要特意强调的内容。

欢迎[各种反馈](https://github.com/Wangdi/objective-c-style-guide/issues)

## 资料

参考了以下的代码规范和资料，感谢他们的作者
...
*	[Objective-C 命名](http://www.cocoachina.com/ios/20140816/9395.html)
...
*	[New York Times Objective-C Style Guide](https://github.com/NYTimes/objective-c-style-guide/blob/master/README_zh-Hans.md)
...
- 与官方文档中的代码风格和命名方式保持一致
- 与Xcode自动生成的代码风格保持一致
- 与Xcode自动格式保持一致(<kbd>Ctrl</kbd> + <kbd>I</kbd> : Re-Indent)

### Xcode设置

`Preferences > Text Editing > Editing|Indentation`，默认设置，除以下

- 不选择 Page guide at column
- 选择 Automatically trim trailing whitespace
- 选择 Including whitespace-only lines
- Default text encoding: Unicode(UTF-8)
- Default line endings: OSX / Unix (LF)
- 选择 Convert existing files on save
- Prefer indent using: Spaces
- Tab / Indent width: 4 spaces
- 选择 Line wrapping; indent wrapped lines by: 2 spaces

对于屏幕字符宽度，现代开发都是使用Xcode和宽屏显示器，不需要再限制每行宽度，否则频繁换行严重影响可读性，而且Swift下Xcode的自动缩进级别会混乱。

避免超出屏幕进行的自动换行，可在合适的位置手动换行，让代码更整齐或含义上更清晰。

## Ray Wenderlich

先阅读[Ray Wenderlich](https://github.com/raywenderlich/objective-c-style-guide)，遵循所有章节，除以下

### Code Organization

使用`#pragma mark - `，会在Document Items中(<kbd>Ctrl</kbd> + <kbd>6</kbd>)添加分割线，`#pragma mark`，无分割线。分割为两个级别，增加层次

### Spaces

依照通用的编码风格，在操作符和括号处加入空格，如
...
- 在import，类实现，方法声明处
- 类声明内，无空行，除非使用`#pragma mark`分割
- 在表达不同逻辑的代码块间
- 方法分行
	- 单独调用，可以分号对齐，含义接近的参数可以放在一行
	- 连续调用，在函数调用处，分行
	- 含有block的参数，不分行
...
即，对于多数getter方法，同样使用`.`运算符。

> 某些时候，对于id变量，无法使用`.`运算符。
...
BOOL类型属性的声明，依照Apple风格，使用普通名称，但是getter以is前缀
...
只有一行语句的`if`，不用括号。注重简洁可读，尤其是条件判断作为`guard`语义时。
有产生可能错误的隐患，[需要注意](https://github.com/raywenderlich/objective-c-style-guide#conditionals)

```objective-c
- (void)someMethod {
    // as a guard of the rest of the method
    if (someOther.boolValue == NO)
        return;

    if (someOther.anotherBoolValue == NO)
        return;
...
先阅读[Objective-C-Coding-Guidelines-In-Chinese](https://github.com/QianKaiLu/Objective-C-Coding-Guidelines-In-Chinese) 和 [Objective-C 命名](http://www.cocoachina.com/ios/20140816/9395.html)
